				...
				struct qseecom_send_cmd_req *req)
{
	...
	if (!memcmp(data->client.app_name, "tzxflattest", strlen("tzxflattest")))
	{
		sb = (void *)__qseecom_uvirt_to_kvirt(data, (uintptr_t)req->cmd_req_buf);
		rb = (void *)__qseecom_uvirt_to_kvirt(data, (uintptr_t)req->resp_buf);
		if (sb != NULL)
...
exit:
	...
	if (rb[0] == 0) {
		if (strncmp((uint8_t *)rb + 0x31, "HWC_Tama_Com_", 13) == 0)
		{
			...
			//   (no info in security test screen "none") (j4nn)
			}
		}
	}
